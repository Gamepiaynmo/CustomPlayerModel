...
public class GuiModelSelection extends Screen {
    ...
    protected void init() {
        ...
        searchInput = new TextFieldWidget(minecraft.fontRenderer, left, bottom + 2, entryWidth + scrollWidth, itemHeight, new StringTextComponent(""));
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (selected >= 0) {
            for (ITextComponent str : searchResult.get(selected).getModelInfo())
                drawCenteredString(matrixStack, minecraft.fontRenderer, str, entityX, entityY + 50 + (deltaY += 10), 0xffffffff);
        }
        ...
        for (int i = Math.min(scrollPos + itemCount, searchResult.size()) - 1; i >= scrollPos; i--) {
            ...
            List<IReorderingProcessor> str = minecraft.fontRenderer.trimStringToWidth(new StringTextComponent(model.info.getName()), entryWidth - 7);
            int color = model.isLocal ? model.isEditing ? 0xffffff55 : 0xffaaaaaa : 0xffffffff;
            minecraft.fontRenderer.func_238422_b_(matrixStack, str.get(0), left + 1, itemTop + 1, color);
    ...
    private void setSelected(int index, int button) {
        if (index >= 0 && index != selected) {
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                Minecraft.getInstance().ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent("gui.cpm.clipboard"));
            }
        }
