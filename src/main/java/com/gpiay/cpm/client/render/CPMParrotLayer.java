        ...
        EntityType.byKey(compoundnbt.getString("id")).filter((p_215344_0_) -> {
            ...
            for (ModelBone.Instance bone : model.getAttachments(leftShoulderIn ? EnumAttachment.PARROT_LEFT : EnumAttachment.PARROT_RIGHT)) {
                if (bone.isVisible()) {
                    ...
                    IVertexBuilder ivertexbuilder = bufferIn.getBuffer(orinLayer.parrotModel.getRenderType(ParrotRenderer.PARROT_TEXTURES[compoundnbt.getInt("Variant")]));
                    orinLayer.parrotModel.renderOnShoulder(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, limbSwing, limbSwingAmount, netHeadYaw, headPitch, entitylivingbaseIn.ticksExisted);
                    matrixStackIn.pop();
                }
            }
