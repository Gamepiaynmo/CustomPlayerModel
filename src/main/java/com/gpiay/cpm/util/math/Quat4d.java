...
public class Quat4d {
    ...
    public final static Quat4d One = new Quat4d(1, 1, 1, 1);
    public final static Quat4d Zero = new Quat4d(0, 0, 0, 0);
    ...
    public double getPitchRad () {
        final int pole = getGimbalPole();
        return pole == 0 ? Math.asin(MathHelper.clamp(2f * (w * x - z * y), -1f, 1f)) : (double)pole * Math.PI * 0.5f;
    }
...
// method
    ...
    public Quat4d setFromCross (final Vector3d v1, final Vector3d v2) {
        final double dot = MathHelper.clamp(v1.dot(v2), -1f, 1f);
    ...
    public Quat4d setFromCross (final double x1, final double y1, final double z1, final double x2, final double y2, final double z2) {
        final double dot = MathHelper.clamp(Vector3d.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
        final double angle = Math.acos(dot);
...
// value
    ...
    public double getAngleAroundRad (final double axisX, final double axisY, final double axisZ) {
        ...
        return 2.0 * Math.acos(MathHelper.clamp(
                (d < 0 ? -this.w : this.w) / Math.sqrt(l2), -1f, 1f));
    }
